...
class ReacherContextualEnv(ReacherEnv):
    ...
    def change_context(self, context_vector):
        """The context vector is a 2-dim vector [x,y] where x, y are the coordinates of the target
        """
        qpos = self.np_random.uniform(low=-0.1, high=0.1, size=self.model.nq) + self.init_qpos
        qpos[-2:] = context_vector
...
if __name__ == "__main__":
    ...
    for i_episode in range(500):
        env.reset()
        while True:
            goal = np.random.uniform(low=-.25, high=.25, size=2)
